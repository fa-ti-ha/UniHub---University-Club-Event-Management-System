...
function initRegisterValidation() {
    ...
    form.addEventListener('submit', async e => {
        ...
        try {
            const res  = await fetch(window.BASE_URL + '/api/auth.php', { method: 'POST', body: formData });
            const data = await res.json();
            if (data.success) {
                showToast('Account created! Redirecting...', 'success');
                setTimeout(() => window.location.href = data.redirect || window.BASE_URL + '/pages/login.php', 1200);
            } else {
...
function validateRegisterForm(form) {
    ...
    for (const [id, rules] of Object.entries(fields)) {
        ...
        if (rules.required && !val) { markInvalid(el, `${rules.label} is required.`); valid = false; continue; }
        if (rules.min && val.length < rules.min) { markInvalid(el, `${rules.label} must be at least ${rules.min} characters.`); valid = false; continue; }
        if (rules.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) { markInvalid(el, 'Enter a valid email address.'); valid = false; continue; }
        if (rules.match) {
            const other = form.querySelector(`[name="${rules.match}"]`);
            if (other && val !== other.value) { markInvalid(el, 'Passwords do not match.'); valid = false; continue; }
        }
        markValid(el);
    }
    return valid;
...
function validateField(input) {
    ...
    if (!val && input.required) { markInvalid(input, 'This field is required.'); return; }
    if (name === 'email' && val && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(val)) { markInvalid(input, 'Enter a valid email.'); return; }
...
function markInvalid(el, msg) {
    ...
    const err = el.parentElement.querySelector('.form-error') || el.closest('.form-group')?.querySelector('.form-error');
